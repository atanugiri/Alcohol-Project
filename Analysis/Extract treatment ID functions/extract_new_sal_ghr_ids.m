...
startDate = datetime('05/01/2024', 'InputFormat', 'MM/dd/yyyy');
% endDate = datetime('04/10/2024', 'InputFormat', 'MM/dd/yyyy');
% endDate = endDate + days(1);

dataInRange = allDates(allDates.referencetime >= startDate, :);
...
"AND UPPER(subjectid) <> UPPER('none') AND UPPER(health) = 'SA' AND " + ...
"notes <> 'No Injection' ORDER BY id", idList);
...
"subjectid, gender, notes FROM live_table WHERE id IN (%s) " + ...
"AND UPPER(subjectid) <> UPPER('none') AND UPPER(health) = 'GHR' AND " + ...
"notes <> 'No Injection' ORDER BY id", idList);
