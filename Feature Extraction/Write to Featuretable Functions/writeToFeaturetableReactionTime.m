...
for index = 1:length(idList)
    ...
    try
        ...
        % Handle empty values
        reactionTime = handleEmpty(reactionTime);

        % Convert NaN values to 'NULL' for text columns
        reactionTime = convertToString(reactionTime);

        updateQuery = sprintf("UPDATE %s SET reaction_time=%s WHERE id=%d", ...
            tableName, reactionTime, id);
